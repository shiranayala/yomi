  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
        <div style={{
          fontFamily: T.fonts.hand, fontSize: 20, color: tone.text, lineHeight: 1.15,
        }}>{n.title}</div>
        {n.pinned && <Icon.pin size={15} color={tone.text} />}
      </div>
      {n.body && (
        <div style={{
          fontSize: 13.5, color: tone.text, opacity: 0.82,
...
export function NotesScreen({ notes, onAdd }: {
  notes: Note[];
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>{colA.map(n => <NoteCard key={n.id} n={n} />)}</div>
        <div style={{ flex: 1 }}>{colB.map(n => <NoteCard key={n.id} n={n} />)}</div>
      </div>

      {notes.length === 0 && !adding && (
        <div style={{
          textAlign: 'center', padding: '60px 20px',
          color: T.color.textMuted, fontSize: 15,
        }}>
